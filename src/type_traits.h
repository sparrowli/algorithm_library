#ifndef ALGORITHM_LIBRARY_SRC_TYPE_TRAITS_H_
#define ALGORITHM_LIBRARY_SRC_TYPE_TRAITS_H_

namespace rtc {

// Determines if the given class has zero-argument .data() and .size() methods
// whose return values are convertible to T* and szie_t, respectively.
...
struct IsIntEnum {
 private:
  // This overload is used if the given type is an enum that converts implicitly to
  // an integral type. 
  template <typename X,
            typename std::enable_if<
                std::is_enum<X>::value &&
...
#endif  // ALGORITHM_LIBRARY_SRC_TYPE_TRAITS_H_